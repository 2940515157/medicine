<%--
  Created by IntelliJ IDEA.
  User: weikang
  Date: 2020/8/26
  Time: 16:23
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>Title</title>
</head>
<body>
<span style="font-weight: bolder;font-size: 25px;margin-left: 300px;">入库申请</span><br><br>
状态:
<select id="status1" class="easyui-combobox" style="width:200px;">
    <option value="-1">请选择</option>
    <option value="0">待审核</option>
    <option value="1">已批准</option>
    <option value="2">未批准</option>
</select>
<a id="btnSearchRequest" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-search'">查询</a>

<table id="dgvRequest" align="center">
</table>

<script type="text/javascript">
    //初始化绑定数据
    var dgvRequestParams = {
        url: 'Stock/fenYe.action',
        columns: [[
            {field: 'checked', checkbox: true, title: '全选'},
            {field: 'stockId', title: 'ID', width: 50, align: 'center'},
            {
                field: 'name', title: '名称', width: 100, align: 'center', formatter: function (value, rows) {
                    return rows.goods.name;
                }
            },
            {field: 'quantity', title: '数量', width: 100, align: 'center'},
            {field: 'time', title: '日期', width: 100, align: 'center'},
            {
                field: 'price', title: '价格', width: 100, align: 'center', formatter: function (value, rows) {
                    return rows.goods.price;
                }
            },
            {field: 'accepter', title: '质检人', width: 100, align: 'center'},
            {
                field: 'status', title: '状态', align: 'center', formatter: function (value, rows) {
                    if (rows.status == 0) {
                        return '<font color="blue">待审核</font>';
                    } else if (rows.status == 1) {
                        return '<font color="green">已批准</font>';
                    } else if (rows.status == 2) {
                        return '<font color="red">未批准</font>';
                    }
                }
            },
            {
                field: 'caozuo', title: '操作', width: '100', align: 'center', formatter: function (value, rows) {
                    if (rows.status == 0) {
                        return '<button class="requestListUp">批准</button>' +
                            '<button class="requestListDown">不批</button> ';
                    }
                }
            }
        ]],
        pagination: true,  //分页工具栏 显示
        singleSelect: true, //只能选一行
        pageList: [5, 10, 15, 20]
    };
    $("#dgvRequest").datagrid(dgvRequestParams);

    //查询
    $("#btnSearchRequest").click(function () {
        $("#dgvRequest").datagrid('load', {status: $("#status1").val()});
    });

    //批准
    $(document).on('click', ".requestListUp", function (e) {
        var selrows = $("#dgvRequest").datagrid('getSelections');
        $.get("Stock/pizhun.action", {stockId: selrows[0].stockId}, function (data) {
            $.messager.show({
                title: '5秒后自动关闭',
                msg: data == true ? "操作成功" : "操作失败",
                timeout: 5000,
                showType: 'slide'
            });
            //刷新表格
            $("#dgvRequest").datagrid('reload', {status: $("#status1").val()});
        });

    });
    //不批
    $(document).on('click', ".requestListDown", function (e) {
        var selrows = $("#dgvRequest").datagrid('getSelections');
        $.get("Stock/bupi.action", {stockId: selrows[0].stockId}, function (data) {
            $.messager.show({
                title: '5秒后自动关闭',
                msg: data == true ? "操作成功" : "操作失败",
                timeout: 5000,
                showType: 'slide'
            });
            $("#dgvRequest").datagrid('reload', {status: $("#status1").val()});
        });
    });
</script>
</body>
</html>
